(function() {
    redirect.idNotFound();
    redirect.categoryExist();

    function titlePageChange(el) {
        var categoryId = parseInt(location.search.split('=').slice(1)[0]);

        for (var i = 0; i < Model.date.Tests_categories.length; i++) {

            if (Model.date.Tests_categories[i].id === categoryId) {

                if (Model.date.Tests_categories[i].parent_id === 0) {
                    el.innerHTML = 'Загальна статистика категорії: ' + Model.date.Tests_categories[i].name;
                } else {
                    el.innerHTML = 'Загальна статистика підкатегорії: ' + Model.date.Tests_categories[i].name;
                }
            };
        };
    }
    ...
    titlePageChange(document.getElementsByTagName('h3')[0]);


    function generalStatisticModule() {


        function formatArray(id, array) {
            var newArray = [];
            for (var i = 0; i < array.length; i++) {
                if (array[i].category === id || array[i].subcategory === id) {
        ...
        function formatResult(array, array2) {
            var newArray = [];
            for (var i = 0; i < array.length; i++) {
                for (var j = 0; j < array2.length; j++) {
                    if (array[i].test_id === array2[j].id) {
                        newArray.push(array[i]);
                    };
                };
        ...
        function formatUsers(array, array2) {
            var newArray = [];
            for (var i = 0; i < array.length; i++) {
                for (var j = 0; j < array2.length; j++) {
                    if (array[i].u_id === array2[j].id) {
        ...
        function categoryData() {
            var categoryId = parseInt(location.search.split('=').slice(1)[0]);
            var categoryData = {};
            categoryData.Tests = formatArray(categoryId, Model.date.Tests);
            categoryData.Result = formatResult(Model.date.Result, categoryData.Tests);
            categoryData.Users = formatUsers(categoryData.Result, Model.date.Users);
            categoryData.Tests_categories = Model.date.Tests_categories;
            categoryData.User_roles = Model.date.User_roles;
            categoryData.Ranks = Model.date.Ranks;
            return categoryData;
        };

        //--------------------------------------------------------------------//
        workDate = categoryData();
        ...
        if (tabs[0].className === 'tabs active') {
            ...
            graphicTitles('Кількість пройдених тестів', 'По часу', 'Кількість тестів', 'шт');
        ...
        function fillSeries() {
            for (var i = 0; i < workDate.Result.length; i++) {
                if (dateInterval.startMiliseconds < workDate.Result[i].passed_date && workDate.Result[i].passed_date < dateInterval.endMiliseconds) {
                    categories[0].push(workDate.Result[i]);
                };
            };
        };
        ...
        function fillSeries2() {
            for (var i = 0; i < workDate.Result.length; i++) {
                if (dateInterval.startMiliseconds < workDate.Result[i].passed_date && workDate.Result[i].passed_date < dateInterval.endMiliseconds) {
                    for (var k = 0; k < workDate.Users.length; k++) {
                        if (workDate.Result[i].u_id === workDate.Users[k].id) {
                            var valDate = workDate.Users[k].birthday.split('-');
                            ...
                            } else if (checkboxes[0].checked && 19 <= res && res <= 25) {

                                categories[0].push(workDate.Result[i]);

                            } else if (checkboxes[0].checked && 25 <= res) {

                                categories[0].push(workDate.Result[i]);

                            } else {

                                console.log('не коректно вказано вік ' + workDate.Users[k].firstName + ' ' + workDate.Users[k].lastName)
                            };
                        };
                    };
                };
        ...
        function fillSeries3() {
            for (var i = 0; i < workDate.Result.length; i++) {
                if (dateInterval.startMiliseconds < workDate.Result[i].passed_date && workDate.Result[i].passed_date < dateInterval.endMiliseconds) {
                    ...
                    if (checkboxes[0].checked && workDate.Result[i].user_rank === 1) {

                        categories[0].push(workDate.Result[i]);

                    } else if (checkboxes[1].checked && workDate.Result[i].user_rank === 2) {

                        categories[0].push(workDate.Result[i]);

                    } else if (checkboxes[0].checked && workDate.Result[i].user_rank === 3) {

                        categories[0].push(workDate.Result[i]);

                    } else if (checkboxes[0].checked && workDate.Result[i].user_rank === 4) {
        ...
        function activeCheckboxChecking(className) {
            ...
            for (var i = 0; i < checkboxes.length; i++) {
                ...
                if (className === 'ageList-checkbox') {
                    ...
                    createSeriesElem(workDate.Ranks[i].name);
                };
            };
        }
        ...
        function dateSortByCategories() {
            ...
            /*        var roots = workDate.Categories.getSubcategories(null);
                    var root_subcategories = [];
                    for (var i = roots.length - 1; i >= 0; i--) {
                        root_subcategories[i] = workDate.Categories.getSubcategories(roots[i]);
                    };*/

            for (var i = 0; i < workDate.Tests.length; i++) {

                /*            for (var j = 0; j < roots.length ; j++) {
                                if (root_subcategories[j].indexOf(workDate.Tests[i].subcategory) >= 0) {
                                    
                                    for (var k = 0; k < workDate.Tests[i].passed_date.length; k++) {
                                        categories[j].push(workDate.Tests[i].passed_date[k]);
                                    };
                ...
                if (workDate.Tests[i].category === 1) {

                    for (var k = 0; k < workDate.Tests[i].passed_date.length; k++) {
                        categories[0].push(workDate.Tests[i].passed_date[k]);
                    };


                } else if (workDate.Tests[i].category === 2) {

                    for (var l = 0; l < workDate.Tests[i].passed_date.length; l++) {
                        categories[0].push(workDate.Tests[i].passed_date[l]);
                    };

                } else if (workDate.Tests[i].category === 3) {

                    for (var m = 0; m < workDate.Tests[i].passed_date.length; m++) {
                        categories[0].push(workDate.Tests[i].passed_date[m]);
                    };
        ...
        function monthParser() {
            ...
            if (startDateObj.startYear !== endDateObj.endYear) {
    ...
    changeCssOnElements = function() {
        ...
        function fillGradeList() {
            ...
            for (var i = 0; i < labels.length; i++) {
                ...
                if (i === workDate.Ranks[i].id - 1) {
                    labels[i].innerHTML += workDate.Ranks[i].name;
                };
            };
        }
        ...
        function buildTableElements(newTestsArrElement, i, tableSelector) {
            ...
            for (var k = 0; k < newTestsArrElement.length; k++) {
                for (var m = 0; m < workDate.Tests.length; m++) {
                    if (newTestsArrElement[k].test_id === workDate.Tests[m].id) {
                        elem5.innerHTML = workDate.Tests[m].subcategory;
                        for (var l = 0; l < workDate.Tests_categories.length; l++) {
                            if (workDate.Tests[m].subcategory === workDate.Tests_categories[l].id) {
                                elem6.innerHTML = workDate.Tests_categories[l].parent_id;
                            };
                        };
                    };
                };
