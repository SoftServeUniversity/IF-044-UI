window.onload = function() {
	var advancedSearch = Object.create(simpleSearch);
	console.log(advancedSearch);
	advancedSearch.showAllcontaining = function(value, category, sub_category) {
        var filter = false;
	...
	// advancedSearch.EventKeyUp = function(){
		// alert(this.row_results);
	// }
	//advancedSearch.init = function(){this.EventKeyUp};
	advancedSearch.init;
	console.log(advancedSearch);
        ...
        // if (result_array.length) {
            // for (ind in result_array) {
                ...
                // for (var j = 0; j < Tests[i].tags.length; j++) {
                    ...
                    // }
                // }
                // row_results.innerHTML += '<div class="col-xs-12 col-sm-9 col-md-9 col-lg-9 search-result well"><h3 class="title-post-name"><a style="text-decoration: none;" href="Test.html?id='+Tests[i].id+'">' + Tests[i].name + '</a></h3><p class="text-justify ">' + Tests[i].description + '</p><div class="col-xs-12"><ul class="search-teg">' + tags + '</ul></div></div>';
            // }
        // } else {
            // showEmpty();
        // }
