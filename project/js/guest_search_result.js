	...
	{ // checking users permission
	  var getUsersPermission = function(session_user_id)
	  {
	    ...
	    else
	    {
	      ...
	      {
	        if (Model.date.Users[i].id == session_user_id && Model.date.Users[
	          i].role_id == 2)
	        {
	          userRole = true;
	          break;
	...
	window.onload = function()
	{
	  ...
	  var generateHtml = {
	    ...
	    { // method which print on page test
	      var content = '';
	      content +=
	        ...
	        '].name">' + this.catchTestById(testId).name + '</h3>';
	      ...
	      for (var i = 0; i < this.catchTestById(testId).question.length; i++)
	      {
	        ...
	        content +=
	          ...
	          Model.date.Tests.indexOf(this.catchTestById(testId)) +
	          '].question[' + i + '].text">' + this.catchTestById(testId).question[
	            i].text + '</div>';
	        for (var j = 0; j < this.catchTestById(testId).answers.length; j++)
	        {
	            ...
	            question_id)
	          {
	            content +=
	              ...
	              Model.date.Tests.indexOf(this.catchTestById(testId)) +
	              '].answers[' + j + '].text_answer">' + this.catchTestById(
	                testId).answers[j].text_answer + '</div>';
	          }
	        }
	        content += '</div>';
	      }
	    ...
	    { // method which print on page tests listCategories
	      var list_option = '';
	      for (var i = 0; i < Model.date.Tests_categories.length; i++)
	      {
	        if (Model.date.Tests_categories[i].parent_id == 0)
	        {
	          if (this.catchTestById(testId).category == Model.date.Tests_categories[
	            i].id)
	          {
	            list_option += '<option value="' + Model.date.Tests_categories[
	                i].id + '" selected>' + Model.date.Tests_categories[i].name +
	              '</option>'
	          }
	          else
	          {
	            list_option += '<option value="' + Model.date.Tests_categories[
	                i].id + '">' + Model.date.Tests_categories[i].name +
	              '</option>'
	          }
	        }
	      }
	    ...
	    { // method which print on page tests listSubCategories
	      var list_option = '';
	      var categoryId;
	      categoryId = (categoryId) ? categoryId : this.catchTestById(testId)
	        .category;
	      ...
	      for (var i = 0; i < Model.date.Tests_categories.length; i++)
	      {
	        ...
	        {
	          if (this.catchTestById(testId).subcategory == Model.date.Tests_categories[
	            i].id)
	          {
	            list_option += '<option value="' + Model.date.Tests_categories[
	                i].id + '" selected>' + Model.date.Tests_categories[i].name +
	    ...
	    { // method which print on page comments created by moderator
	      ...
	      if (Model.date.comment)
	      {
	        for (var i = 0; i < Model.date.comment.length; i++)
	        {
	          ...
	          {
	            newDiv.innerHTML += "<b>" + Model.date.comment[i].comment +
	              "</b><br />" + "<div class='text-left'><i>" + Model.date.comment[
	                i].timeCreate + " Moderator</i></div>";
	          }
	        }
	        this.place_for_comments.insertBefore(newDiv, document.getElementById(
	          "add-comment").parentNode);
	      }
	    },
	    init: function(get_test_id)
	    { //method which call all needed  methods for generation html
	  ...
	  function wrote(testId, textComment)
	  {
	    ...
	    if (!dublicate)
	    {
	      ...
	      Model.date.Comment[Model.date.Comment.length - 1].Date_creation.push(
	        c);
	  ...
	  var Events = {
	      ...
	      for (var i = 0; i < this.content_for_edit.length; i++)
	      {
	        this.content_for_edit[i].addEventListener('dblclick', function(e)
	        {
	          ...
	          function saveChange(e)
	          {
	            ...
	            if (e.target.className.indexOf('edit-textarea') &&
	              changeText.length > validateLength)
	            {
	              that.style.display = "block";
	              document.querySelectorAll('.edit-textarea')[0].parentNode
	                .removeChild(document.querySelectorAll(
	                  '.edit-textarea')[0]);
	              that.textContent = changeText;
	              eval(test + '="' + changeText + '"');
	              document.querySelectorAll('body')[0].removeEventListener(
	                'click', saveChange);
	            }
	            else
	            {
	              document.querySelectorAll('.edit-textarea')[0].style
	                .border = (changeText.length <= validateLength) ?
	                'solid 1px red' : '';
	    ...
	    {
	      generateHtml.place_for_list_categories.addEventListener('change',
	        function()
	        {
	    ...
	    addComents: function()
	    {
	      document.getElementById('add-comment').addEventListener('submit',
	        function()
	        {
	          var element = document.querySelectorAll('.div-list-comments')[
	            0];
	          if (element)
	          {
	          ...
	          month[10] = "лис";
	          month[11] = "гру";
	          var timeCreateComments = day.getDate() + " " + month[day.getMonth()] +
	            " о " + day.getHours() + ":" + minutes;
	          ...
	          generateHtml.catchTestById(get_test_id).category =
	            generateHtml.place_for_list_categories.value;
	          generateHtml.catchTestById(get_test_id).subcategory =
	            generateHtml.place_for_list_subCategories.value;
	    ...
	    {
	      document.getElementById('unpublic').addEventListener('click',
	        function()
	        {
	          generateHtml.catchTestById(get_test_id).status = 2;
	          generateHtml.catchTestById(get_test_id).category =
	    ...
	    saveTags: function()
	    {
	      ...
	      console.log(document.getElementById('tags').value +
	        "should be tags");
	      generateHtml.catchTestById(get_test_id).tags = [].concat(document.getElementById(
	        'tags').value.split(','));
	      console.log(generateHtml.catchTestById(get_test_id).tags);
	    },
	    saveDescription: function()
	    {
	      generateHtml.catchTestById(get_test_id).description = generateHtml
	        .textarea_for_description.value;
	    },
	    init: function()
	    {
